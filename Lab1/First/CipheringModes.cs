...
class ECBCipheringMode : ICipheringMode
{
    ...
    public byte[] decipher(in byte[] dataToDecipher, in byte[]? IV = null, bool isFinalBlock = true)
    {
        int blockSize = cipheringAlgorithm.BlockSize;
    ...
    public byte[] decipher(in byte[] dataToDecipher, in byte[] IV, bool isFinalBlock = true)
    {
        ...
        Parallel.For(
            ...
            blockIndex =>
            {
                ...
                byte[] decryptedBlock = cipheringAlgorithm.decipherBlock(in localBlockToDecipher);
...
class OFBCipheringMode : ICipheringMode
{
    ...
    public byte[] decipher(in byte[] dataToDecipher, in byte[] IV, bool isFinalBlock = true)
    {
        ...
        int blockSize = cipheringAlgorithm.BlockSize;
        if (isFinalBlock && dataToDecipher.Length != dataToDecipher.Length / blockSize * blockSize)
        {
