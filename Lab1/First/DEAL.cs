public class DEALRoundKeys : IGetRoundKeys
{
    ...
    private byte[][] roundsFor128BitKey(in byte[] KKey, in byte[][] keyBlocks)
    {
        ...
        for (int i = 2; i < 6; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                blockToCipher[j] = (byte)(keyBlocks[i % 2][j] ^ roundKeys[i - 1][j] ^ upperBit[j]);
            }
    ...
    private byte[][] roundsFor192BitKey(in byte[] KKey, in byte[][] keyBlocks)
    {
        ...
        for (int i = 3; i < 6; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                blockToCipher[j] = (byte)(keyBlocks[i % 3][j] ^ roundKeys[i - 1][j] ^ upperBit[j]);
            }
            roundKeys[i] = cbc.cipher(in blockToCipher, IV);
        }
        return roundKeys;
    ...
    private byte[][] roundsFor256BitKey(in byte[] KKey, in byte[][] keyBlocks)
    {
        ...
        for (int i = 4; i < 8; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                blockToCipher[j] = (byte)(keyBlocks[i % 4][j] ^ roundKeys[i - 1][j] ^ upperBit[j]);
            }
            roundKeys[i] = cbc.cipher(in blockToCipher, IV);
        }
        return roundKeys;
